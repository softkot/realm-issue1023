...
import io.realm.issue1023.entity.TestEntity;
...
public class ReportActivity extends Activity {
    ...
    Runnable dataWorker = new Runnable() {
        @Override
        public void run() {
            realm.beginTransaction();
            for (TestEntity testEntity : realm.allObjects(TestEntity.class)) {
                testEntity.removeFromRealm();
            }
            for (int i = 0; i < 100; i++) {
